module Doorkeeper
  # overide tokens controller doorkeeper
  class TokensController < Doorkeeper::ApplicationMetalController
    before_action :validate_presence_of_client, only: [:revoke]

    def create
      headers.merge!(authorize_response.headers)
      render json: authorize_response.body.merge({ data: @data }),
    ...
    def token
      @token ||= if params[:token_type_hint] == 'refresh_token'
    ...
    def assign_user_info(context)
      return if context.auth.try(:token).nil?

      resource = context.auth.token
      owner_id = resource.resource_owner_id
      @data = User.find owner_id
    end
